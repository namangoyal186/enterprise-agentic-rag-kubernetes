...
# 1. Clean token and base_url strings
raw_token = settings.LOGFIRE_TOKEN or ""
clean_token = raw_token.strip('"').strip("'").strip()

target_base_url = (settings.LOGFIRE_BASE_URL or "https://logfire-us.pydantic.dev").strip('"').strip("'").strip()

# 2. Configure Logfire dynamically
if clean_token:
    logfire.configure(
        token=clean_token,
        service_name="enterprise-ingestion-service",
        send_to_logfire=True,
        advanced=logfire.AdvancedOptions(base_url=target_base_url),
    )
else:
    logfire.configure(
        service_name="enterprise-ingestion-service",
...
# Add this print statement temporarily to verify setting resolution:
print(f"Connecting to Qdrant at URL: {settings.QDRANT_URL}")
